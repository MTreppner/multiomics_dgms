...
args.experiment='rna_atac'
args.model='rna_atac'
args.obj='m_elbo_naive_warmup'
args.batch_size=64
args.epochs=100 # 50
args.deterministic_warmup=50 # 25
args.lr=1e-4
args.latent_dim=10
args.num_hidden_layers=3
args.r_hidden_dim=500
args.p_hidden_dim=100
args.learn_prior
...
def run(i,j):
    ...
    if args.analytics:
        def get_latent(dataloader, train_test, runPath):
            ...
            mean_lats.to_csv('{}/csv/latent_subsample_{}_cells_rep_{}.csv'.format(runPath,j, rep))
    ...
    # save timings 
    with open(runPath+'/timings_%s_cells_rep_%s.txt'%(j,i), 'w') as f: 
        f.write('clocktime: %s \n'%(clocktime_elapsed))
        f.write('cputime: %s \n'%(cputime_elapsed))
    f.close()
